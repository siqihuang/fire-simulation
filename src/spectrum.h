...
public:
	...
	Spectrum()
	{
		bbTemp = 5000;                 /* Hidden temperature argument
											 to BB_SPECTRUM. */
		tmin = 1000.0;
		tmax = 10000.0;
		transderFac = (tmax - tmin) / (Tmax - Tignition);
		//transderFac = (tmax - tmin) / (Tmax - 0);
	}
	~Spectrum()
	{
	}
	...
	inline double temperatureTransfer(double temperature) 
	{
		//printf("%lf ", temperature);
		return tmin + (temperature - Tignition) * transderFac; 
		//return tmin + (temperature - 0) * transderFac; 
	}
	/*                          SPECTRUM_TO_XYZ
	...
	*/
	...
	//void spectrum_to_xyz(double (Spectrum::*spec_intens)(double wavelength),
	...
	*/
	...
	glm::dvec3 getRGB(double temperature)
	{
		double x, y, z, r, g, b;
		bbTemp = temperatureTransfer(temperature);
